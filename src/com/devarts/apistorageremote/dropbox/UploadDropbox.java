...
//            System.out.println("Uploaded:"+metadata.getName()+"-"+metadata.getSharingInfo());

        } catch (UploadErrorException ex) {
            System.err.println("Error uploading to Dropbox3: " + ex.getMessage());
            System.exit(1);
        } catch (DbxException ex) {      	
            System.err.println("Error uploading to Dropbox4: " + ex.getMessage());
            System.err.println("TESTE: "+ex.getLocalizedMessage());
            System.exit(1);
        ...
        for (int i = 0; i < CHUNKED_UPLOAD_MAX_ATTEMPTS; ++i) {
            ...
            try (InputStream in = new FileInputStream(localFile)) {
                ...
                if (ex.errorValue.isLookupFailed() && ex.errorValue.getLookupFailedValue().isIncorrectOffset()) {
                    ...
                    System.err.println("Error uploading to Dropbox1: " + ex.getMessage());
                    System.exit(1);
                    return;
                }
            } catch (DbxException ex) {
                System.err.println("Error uploading to Dropbox2: " + ex.getMessage());
                System.exit(1);
                return;
            } catch (IOException ex) {
                System.err.println("Error reading from file \"" + localFile + "\": " + ex.getMessage());
    ...
    public void upload(String localPath, String dropboxPath) throws IOException {
